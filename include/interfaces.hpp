...
// Not commented due to assignment specification.
...
struct AxisInterface {
 ...
 protected:
  double value_;
  explicit AxisInterface(double const initial_value) : value_{initial_value} {};
};

struct AnyAxis final : public AxisInterface {
 public:
  explicit AnyAxis(double const initial_value) : AxisInterface{initial_value} {}
  template <class OtherAxis, class = std::enable_if_t<
                                 std::is_base_of_v<AxisInterface, OtherAxis>>>
...
struct XAxis final : public AxisInterface {
 public:
  explicit XAxis(double const initial_value) : AxisInterface{initial_value} {}
};

struct YAxis final : public AxisInterface {
 public:
  explicit YAxis(double const initial_value) : AxisInterface{initial_value} {}
};

struct ZAxis final : public AxisInterface {
 public:
  explicit ZAxis(double const initial_value) : AxisInterface{initial_value} {}
};

struct AllAxis final : public AxisInterface {
 public:
  explicit AllAxis(double const initial_value) : AxisInterface{initial_value} {}
  template <class OtherAxis, class = std::enable_if_t<
                                 std::is_base_of_v<AxisInterface, OtherAxis>>>
  operator OtherAxis() const {
    return OtherAxis{value_};
...
class LocationInterface : public PolymorphicInterface {
 public:
  ...
  auto move_to(XAxis const new_location) { return move_to_imple(new_location); }
  auto move_to(YAxis const new_location) { return move_to_imple(new_location); }
  auto move_to(ZAxis const new_location) { return move_to_imple(new_location); }
 ...
 private:
  virtual auto set_parent_frame_name_imple(std::string frame_name) -> void = 0;
  ...
  virtual auto move_to_imple(XAxis new_x, YAxis new_y, ZAxis new_z) -> void = 0;
...
class YawInterface : public PolymorphicInterface {
 public:
  ...
  [[nodiscard]] auto get_orientation() const { return get_orientation_imple(); }
...
class ColourInterface : public PolymorphicInterface {
 public:
  enum class Colour : unsigned char { red, yellow, green, blue, black, white };
